//
// Created by fonta on 01/12/2022.
//
#include <stdexcept>
#include <ostream>
#include "grafo.h"
...
private:
    ...
    bool inseribile(nodo n);
};

/*
 * CREAZIONE DI UN GRAFO
 * POST CONDIZIONE: Grafo vuoto
 */
template<class E, class P>
...
/*
 * Controlla se un grafo è vuoto
 * POST CONDIZIONE: Return TRUE se il grafo è vuoto, FALSE altrimenti
 */
template<class E, class P>
...
void grafoMat<E, P>::insNodo(nodoG &n) {
    ...
    } else throw out_of_range("NODO GIA ESISTENTE");
 ...
 * PRE CONDIZIONE:Arco tra il nodo u e il nodo v non esiste
...
void grafoMat<E, P>::insArco(const nodo u, const nodo v, const peso p) {
    ...
    if (!esisteArco(a)) {
        ...
        archi++;
    } else throw out_of_range("ARCO ESISTENTE");
}
 ...
 * PRE CONDIZIONE: L'arco tra il nodo u e il nodo v esiste
 ...
 * POST CONDIZIONE: La lista contiere tutti i nodi v per cui esiste un arco uscente da n e entrante in v
 */
template<class E, class P>
...
/*
 * La funzione restituisce true se il nodo esiste, false altrimenti
 */
template<class E, class P>
...
/*
 * La funzione restituisce TRUE se l arco esiste, false altrimenti
 */
template<class E, class P>
bool grafoMat<E, P>::esisteArco(arco a) const {
    ...
    else throw std::out_of_range("UNO DEI NODI NON ESISTE");
};
...
/*
 * Viene restituito il peso dell'arco tra il nodo u e il nodo v (se esiste)
 */
template<class E, class P>
...
/*
 * Viene scritto il peso dell'arco tra il nodo u e il nodo v (se esiste)
 */
template<class E, class P>
...
bool grafoMat<E, P>::inseribile(nodo n) {
    if (n.getId()<0) return true;
    else return matrice[n.getId()].vuoto;
}
