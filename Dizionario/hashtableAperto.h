...
private:
    //numero elementi del dizionario
    int numElementi;
    int divisore;
    //lista di trabocco
...
open_hash_table<K,E>::open_hash_table() {
    numElementi=0;
    divisore=10; //dimensione predefinita
...
open_hash_table<K,E>::open_hash_table(int dim) {
    numElementi = 0;
    divisore = dim; //dimensione decisa dall'utente
    tabella=new List_vector<mypair<K,E>>[divisore];
}
...
/*
 * Verifica se un dizionario è vuoto
 * POST CONDIZIONE: Return TRUE se il dizionario è vuoto, FALSE altrimenti
 */
template <class K, class E>
...
/*
 * Ricava la posizione data una chiava
 */
template <class K,class E>
 ...
 * PRE CONDIZIONE: La chiave non esiste
...
void open_hash_table<K, E>::inserisci( mypair<chiave, valore> &coppia) {
    ...
    if (ricerca(coppia.getChiave())) {
        cout<<"Chiave "<<coppia.getChiave()<<" gia' esistente, ma e' possibile aggiornarne il valore con l'apposita funzione"<<endl;
    }
        else
        {
...
/*
 * Cancella una coppia chiave valore dal dizionario
 * PRE CONDIZIONE: la chiave esiste
 * POST CONDIZIONE: dizionario con coppia eliminata
 */
template <class K,class E>
 ...
 * PRE CONDIZIONE: Esiste la chiave
 * POST CONDIZIONE: Ritorna il valore associato alla chiave
 */
template <class K,class E>
...
//Resituisce TRUE se la chiave key esiste, false altrimenti
template <class K,class E>
...
/*
 * Modifica del valore associato a una chiave esistente
 */
template <class K,class E>
void open_hash_table<K, E>::modifica(const open_hash_table::chiave &key, const open_hash_table::valore &valore, const open_hash_table::valore &nuovoValore){
